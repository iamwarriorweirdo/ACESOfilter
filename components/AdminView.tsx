...
const AdminView: React.FC<AdminViewProps> = ({
    ...
    // Wrapper handlers to stop propagation and show loading
    ...
    return (
        <div className="relative min-h-screen w-full flex flex-col overflow-hidden">
                    ...
                    <div className="glass-panel sticky top-0 z-30 p-4 md:px-8 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-6 shadow-2xl">
                        <div className="flex flex-col lg:flex-row lg:items-center gap-6">
                            <div className="space-y-1">
                                <h2 className="text-3xl font-black text-foreground tracking-tighter bg-clip-text text-transparent bg-gradient-to-r from-foreground to-foreground/60">
                                    {t.welcomeAdmin}
                                </h2>
                                <p className="text-[10px] uppercase tracking-[0.2em] font-bold text-primary/80 px-1">System Intelligence Active</p>
                            </div>

                            <div className="flex bg-muted/20 backdrop-blur-md p-1.5 rounded-2xl border border-white/5">
                        ...
                        <div className="flex items-center gap-4">
                            <div className="flex items-center gap-2 bg-muted/20 rounded-xl p-1 border border-white/5">
                                <button onClick={() => {
                                    const langs: Language[] = ['en', 'vi', 'zh'];
                                    const next = langs[(langs.indexOf(language) + 1) % langs.length];
                                    setLanguage(next);
                                }} className="px-3 py-1.5 rounded-lg hover:bg-muted/40 transition-colors text-muted-foreground flex items-center gap-2 font-black text-[10px] uppercase">
                                    <Globe size={14} className="text-primary" /> {language}
                                </button>
                                <button onClick={toggleTheme} className="p-2 rounded-lg hover:bg-muted/40 transition-colors text-muted-foreground">
                    ...
                    <div className="animate-fade-in space-y-8">
                        {activeTab === 'docs' ? (
                            <div className="space-y-8">
                                    ...
                                    <div className="flex flex-col md:flex-row items-center justify-between gap-10 relative z-10">
                                        <div className="flex items-center gap-8">
                                            ...
                                            <div className="space-y-2">
                                                <h3 className="font-black text-2xl tracking-tight">{t.uploadTitle}</h3>
                                        ...
                                        <label className="relative flex-1 md:max-w-md h-32 border-2 border-dashed border-primary/20 rounded-3xl bg-white/5 dark:bg-black/20 hover:bg-primary/5 hover:border-primary/40 transition-all cursor-pointer group/upload">
                                ...
                                <div className="flex flex-col md:flex-row gap-4 justify-between items-center glass-panel p-2 rounded-2xl border-white/5 shadow-xl">
                                    <div className="flex items-center gap-1.5 px-4 overflow-x-auto no-scrollbar">
                                        ...
                                        {getBreadcrumbs().map((folder) => (
                                            <div key={folder!.id} className="flex items-center gap-1.5 shrink-0">
                                                ...
                                                <button onClick={() => setCurrentFolderId(folder!.id)} className="text-xs font-black uppercase tracking-wider hover:text-primary px-4 py-2 rounded-xl hover:bg-muted/40 transition-all truncate max-w-[120px] sm:max-w-[200px]">{getLocalizedFolderName(folder!.name)}</button>
                                            </div>
                                        ))}
                                        {currentFolderId && <span className="text-[10px] font-black bg-muted/50 px-2 py-0.5 rounded-full text-muted-foreground ml-2">{visibleFiles.length} ITEM(S)</span>}
                                    </div>
                                    <div className="flex items-center gap-3 w-full md:w-auto p-1">
                                        <div className="relative flex-1 md:w-72 group">
                                            <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground group-focus-within:text-primary transition-colors" />
                                            <input type="text" placeholder={t.searchDocs} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-muted/20 border-none rounded-xl pl-12 pr-4 py-3 text-xs font-bold focus:ring-2 focus:ring-primary/20 outline-none h-11 transition-all" />
                                        </div>
                                ...
                                <div className="min-h-[400px] space-y-10">
                                                ...
                                                <div key={folder.id} className="group relative glass-card rounded-3xl p-6 transition-all select-none overflow-hidden aspect-square flex flex-col justify-center items-center">
                                                    ...
                                                    <div className="cursor-pointer flex flex-col items-center text-center gap-4 relative z-10" onDoubleClick={() => setCurrentFolderId(folder.id)}>
                                                        ...
                                                        <div className="w-full space-y-1">
                                                            <div className="font-black text-xs uppercase tracking-tighter truncate w-full" title={getLocalizedFolderName(folder.name)}>{getLocalizedFolderName(folder.name)}</div>
                                                    ...
                                                    <div className="absolute top-2 right-2 flex gap-1 opacity-100 lg:opacity-0 group-hover:opacity-100 transition-all translate-y-2 group-hover:translate-y-0 duration-300">
                                                        <button onClick={(e) => { e.stopPropagation(); setEditingFolderId(folder.id); setEditFolderName(folder.name); }} className="p-2 bg-background/50 hover:bg-blue-500 hover:text-white backdrop-blur-md rounded-xl text-muted-foreground shadow-sm border border-white/5"><Edit3 size={12} /></button>
                                                        <button onClick={(e) => { e.stopPropagation(); handleDeleteFolderWrapper(folder.id); }} className="p-2 bg-background/50 hover:bg-red-500 hover:text-white backdrop-blur-md rounded-xl text-muted-foreground shadow-sm border border-white/5">
                                    ...
                                    <div className="glass-panel rounded-3xl overflow-hidden shadow-2xl border-white/10">
                                        <div className="px-8 py-4 border-b border-border/10 bg-muted/5 flex justify-between items-center text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] relative overflow-hidden">
                                        ...
                                        <div className="divide-y divide-border/10">
                                            {visibleFiles.length === 0 && visibleFolders.length === 0 ? (
                                                <div className="p-32 text-center space-y-6 relative overflow-hidden">
                                                    ...
                                                    <div className="w-20 h-20 bg-muted/10 rounded-full flex items-center justify-center mx-auto opacity-20 relative animate-pulse"><Search size={40} /></div>
                                                    <div className="text-muted-foreground italic font-black text-xs uppercase tracking-[0.3em] opacity-30 relative">{t.dbEmpty}</div>
                                                </div>
                                            ) : (
                                                    ...
                                                    return (
                                                        <div key={doc.id} onDoubleClick={() => onEditDoc(doc)} className={`px-8 py-6 flex flex-col md:flex-row md:items-center justify-between hover:bg-primary/5 transition-all gap-6 group cursor-pointer select-none relative ${isScanning ? 'status-glow-indigo' : isIndexed ? 'status-glow-emerald' : isError ? 'border-red-500/20' : ''}`}>
                                                            ...
                                                            <div className="flex items-center gap-3 self-end md:self-auto opacity-100 lg:opacity-0 group-hover:opacity-100 transition-all translate-x-4 lg:translate-x-12 group-hover:translate-x-0 duration-500 z-10">
                                                                {(isError || isIndexed) && (
                                                                    <button onClick={(e) => { e.stopPropagation(); onEditDoc(doc); }} className="p-3 rounded-2xl hover:bg-orange-500/20 text-orange-500 transition-all border border-white/5 bg-background/50 backdrop-blur-md shadow-lg" title="View Diagnostic Metadata"><Search size={18} /></button>
                                                                )}
                                                                <button onClick={(e) => { e.stopPropagation(); handleDownload(doc); }} className="p-3 rounded-2xl hover:bg-muted/40 text-muted-foreground hover:text-foreground transition-all border border-white/5 bg-background/50 backdrop-blur-md shadow-lg" title="Download Archive"><Download size={18} /></button>
                                                                ...
                                                                <button onClick={(e) => { e.stopPropagation(); handleDeleteDocWrapper(doc.id); }} className="p-3 rounded-2xl hover:bg-red-500/20 text-muted-foreground hover:text-red-500 transition-all border border-white/5 bg-background/50 backdrop-blur-md shadow-lg">
                            ...
                            <div className="h-[800px] glass-panel rounded-[3rem] overflow-hidden shadow-2xl p-4 border-white/5 relative z-10">
            ...
            <div className={`fixed z-50 bg-card/60 backdrop-blur-3xl border-l border-white/10 shadow-[0_0_100px_rgba(0,0,0,0.5)] transform transition-all duration-500 cubic-bezier(0.4, 0, 0.2, 1) flex flex-col ${isChatOpen ? 'translate-x-0' : 'translate-x-full'} ${isChatFull ? 'inset-0 w-full' : 'inset-y-0 right-0 w-full md:w-[500px] lg:w-[35%]'}`}>
                <div className="h-20 border-b border-white/5 flex items-center justify-between px-8 bg-muted/10 shrink-0">
                    ...
                    <div className="flex items-center gap-2">
                        <button onClick={() => setIsChatFull(!isChatFull)} className="p-3 hover:bg-white/5 rounded-xl text-muted-foreground hover:text-white transition-all hidden md:block">{isChatFull ? <Minimize2 size={20} /> : <Maximize2 size={20} />}</button>
                        <button onClick={onNewChat} className="p-3 hover:bg-white/5 rounded-xl text-muted-foreground hover:text-white transition-all" title={t.newChat}><RefreshCcw size={20} /></button>
            ...
            {isCreateFolderOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-xl animate-fade-in">
                    <form onSubmit={submitCreateFolder} className="bg-card/40 glass-panel p-10 rounded-[3rem] w-full max-w-md border-white/10 shadow-2xl space-y-8 animate-slide-up">
                        <div className="flex flex-col items-center text-center gap-4">
                            ...
                            <div className="space-y-1">
                                <h3 className="font-black text-2xl tracking-tighter uppercase">Initialize Archive</h3>
                        ...
                        <input autoFocus type="text" value={newFolderName} onChange={e => setNewFolderName(e.target.value)} placeholder="Sector Name..." className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-5 text-lg font-black tracking-tight focus:ring-4 focus:ring-primary/20 outline-none transition-all placeholder:text-muted-foreground/30" />
            ...
            {editingFolderId && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-xl animate-fade-in">
                    <form onSubmit={submitRenameFolder} className="bg-card/40 glass-panel p-10 rounded-[3rem] w-full max-w-md border-white/10 shadow-2xl space-y-8 animate-slide-up">
                        <div className="flex flex-col items-center text-center gap-4">
                            ...
                            <div className="space-y-1">
                                <h3 className="font-black text-2xl tracking-tighter uppercase">Modify Metadata</h3>
                        ...
                        <input autoFocus type="text" value={editFolderName} onChange={e => setEditFolderName(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-5 text-lg font-black tracking-tight focus:ring-4 focus:ring-blue-500/20 outline-none transition-all" />
                        <div className="flex items-center gap-4 pt-2">
